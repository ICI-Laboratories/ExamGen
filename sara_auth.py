"""Server-side SARA Authorization Code + PKCE support for ExamGen.
...
CLIENT_ID = "examgen-web"
RESOURCE_AUDIENCE = "examgen"
DEVICE_NAME = "ExamGen"
DEFAULT_CALLBACK_URL = "http://localhost:8501/"
ADMIN_CAPABILITIES = frozenset({"examgen.admin", "platform.manage"})
...
_LEGACY_IDENTITY_PATTERN = re.compile(r"^[A-Za-z0-9][A-Za-z0-9@._-]{0,254}$")
...
class IdentityManifestError(AuthError):
    pass
...
class AuthConfig:
    ...
    legacy_identity_manifest: str | None = field(default=None, repr=False)
    ...
    def from_mapping(
        ...
        *,
        ...
        callback_url = value("SARA_AUTH_CALLBACK_URL") or DEFAULT_CALLBACK_URL
        if not portal_url or not identity_url:
        ...
        transaction_db_path = value("SARA_AUTH_TRANSACTION_DB") or str(
            Path(".data") / "examgen_auth_transactions.sqlite3"
        )
        ...
        manifest = value("SARA_LEGACY_IDENTITY_MANIFEST") or None
        return cls(
            ...
            legacy_identity_manifest=manifest,
        )
...
def parse_admin_subjects(raw: Any) -> frozenset[str]:
    """Parse an explicit UUID allowlist; email/name values are never accepted."""
    ...
    if isinstance(raw, str):
        ...
        if stripped.startswith("["):
            try:
                ...
                raise AuthConfigurationError("invalid_admin_subjects") from exc
        else:
            values = [item.strip() for item in stripped.split(",") if item.strip()]
    if not isinstance(values, (list, tuple, set, frozenset)):
        raise AuthConfigurationError("invalid_admin_subjects")
...
def profile_is_admin(
    profile: Mapping[str, Any],
    allowed_subjects: frozenset[str] | set[str] | tuple[str, ...] = (),
) -> bool:
    """Authorize only a verified central capability or explicit UUID."""

    subject = central_subject(profile)
    permissions = profile.get("permissions")
    if isinstance(permissions, (list, tuple, set, frozenset)) and any(
        isinstance(item, str) and item in ADMIN_CAPABILITIES for item in permissions
    ):
        return True
    try:
        normalized_allowlist = {str(uuid.UUID(str(item))) for item in allowed_subjects}
    except (TypeError, ValueError, AttributeError):
        return False
    return subject in normalized_allowlist


def load_identity_manifest(path: str | None) -> dict[str, str]:
    if not path:
        return {}
    try:
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise IdentityManifestError("manifest_unavailable") from exc
    if not isinstance(payload, dict) or payload.get("version") != 1:
        raise IdentityManifestError("manifest_version_invalid")
    mappings = payload.get("mappings")
    if not isinstance(mappings, dict):
        raise IdentityManifestError("manifest_mappings_invalid")

    validated: dict[str, str] = {}
    claimed_identities: set[str] = set()
    for raw_subject, raw_identity in mappings.items():
        try:
            subject = str(uuid.UUID(str(raw_subject)))
        except (TypeError, ValueError, AttributeError) as exc:
            raise IdentityManifestError("manifest_subject_invalid") from exc
        if not isinstance(raw_identity, str) or not _safe_legacy_identity(raw_identity):
            raise IdentityManifestError("manifest_identity_invalid")
        if subject in validated or raw_identity in claimed_identities:
            raise IdentityManifestError("manifest_mapping_duplicated")
        validated[subject] = raw_identity
        claimed_identities.add(raw_identity)
    return validated


def data_identity_for_profile(
    profile: Mapping[str, Any],
    ...
    if not manifest:
        return subject
    legacy_identity = manifest.get(subject)
    if legacy_identity is None:
        return subject
    if not _safe_legacy_identity(legacy_identity):
        raise IdentityManifestError("manifest_identity_invalid")
...
def _safe_legacy_identity(value: str) -> bool:
    return (
        value not in {".", ".."}
        and _LEGACY_IDENTITY_PATTERN.fullmatch(value) is not None
        and "/" not in value
        and "\\" not in value
    )
