...
if not conn:
    st.error("🔴 Error Crítico: Conexión a base de datos no disponible. No se pueden cargar las estadísticas.")
    st.stop()

st.subheader("Resumen General Personal")
try:
    pregunta_mas_errada_info = obtener_pregunta_mas_equivocada_usuario(conn, usuario_id=user_email)
    promedio_tiempo_seg = obtener_promedio_tiempo_respuesta_usuario(conn, usuario_id=user_email)

    col1, col2 = st.columns(2)

    with col1:
        if pregunta_mas_errada_info and 'pregunta_texto' in pregunta_mas_errada_info and pregunta_mas_errada_info['pregunta_texto'] is not None:
            st.metric(
            ...
            st.caption("No se pudo recuperar el texto de la pregunta o no hay errores registrados.")
        else:
            st.info("No hay datos suficientes para determinar tu pregunta más fallada o no has cometido errores.")
    ...
    logger.error(f"Error en UI - Estadísticas Generales Personales para {user_email}: {e}", exc_info=True)
...
try:
    ...
    if not documentos_cargados_raw:
        st.warning("No hay documentos cargados en el sistema para mostrar estadísticas.")
        # st.stop() # Allow rendering rest of page
    else:
        documentos_cargados_sorted = sorted(documentos_cargados_raw, key=lambda x: x['nombre'])
        nombres_documentos_map = {doc["nombre"]: doc["id"] for doc in documentos_cargados_sorted}
        ...
        if not nombres_documentos_map:
            ...
            if doc_nombre_seleccionado:
                documento_id_seleccionado = nombres_documentos_map[doc_nombre_seleccionado]
                st.write(f"Mostrando estadísticas para el documento: **{doc_nombre_seleccionado}** (ID: {documento_id_seleccionado})")
                ...
                if user_doc_stats_data:
                    ...
                    expected_cols = ['pregunta_id', 'pregunta_texto', 'total_respuestas_usuario',
                                     'correctas_usuario', 'incorrectas_usuario', 'tiempo_promedio_usuario_secs']
                    missing_cols = [col for col in expected_cols if col not in df_stats.columns]
                    if missing_cols:
                        st.error(f"Faltan columnas en los datos de estadísticas: {', '.join(missing_cols)}. Verifica la función 'obtener_estadisticas_por_documento_para_usuario'.")
                        st.dataframe(df_stats) 
                        # st.stop()
                    else:
                        df_stats['accuracy_usuario_%'] = df_stats.apply(
                            lambda row: (row['correctas_usuario'] / row['total_respuestas_usuario'] * 100)
                                        if row['total_respuestas_usuario'] > 0 else 0.0, 
                            axis=1
                        ).round(1) 

                        df_stats['tiempo_promedio_usuario_secs'] = pd.to_numeric(df_stats['tiempo_promedio_usuario_secs'], errors='coerce').fillna(0.0)

                        display_df = df_stats.rename(columns={
                        ...
                        st.dataframe(display_df[[
                            'ID Pregunta', 'Pregunta', 'Tus Intentos', 'Correctas', 'Incorrectas',
                            'Tu Tiempo Prom. (s)', 'Tu Precisión (%)'
                        ]], use_container_width=True)

                        st.subheader("Gráficos de Rendimiento Personal (para este documento)")
                        df_answered = df_stats[df_stats['total_respuestas_usuario'] > 0].copy() 

                        if not df_answered.empty:
                            df_answered['pregunta_id_str'] = df_answered['pregunta_id'].astype(str)

                            col_chart1, col_chart2 = st.columns(2)
                            with col_chart1:
                                fig1, ax1 = plt.subplots()
                                ax1.bar(df_answered["pregunta_id_str"], df_answered["tiempo_promedio_usuario_secs"], color='skyblue')
                            ...
                            with col_chart2:
                             ...
                             st.info("No has respondido ninguna pregunta de este documento todavía para generar gráficos.")
                else:
                    st.info(f"No hay estadísticas de preguntas disponibles para ti en el documento '{doc_nombre_seleccionado}'.")
                    st.caption("Esto puede ser porque el documento no tiene preguntas o porque aún no has interactuado con ellas.")

except Exception as e:
    st.error(f"Ocurrió un error al cargar las estadísticas por documento: {e}")
    doc_name_for_log = doc_nombre_seleccionado if 'doc_nombre_seleccionado' in locals() and doc_nombre_seleccionado else 'N/A'
    logger.error(f"Error en UI - Estadísticas por Documento para {user_email}, Doc: {doc_name_for_log}: {e}", exc_info=True)