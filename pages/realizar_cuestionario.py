...
try:
    ...
    if not documentos_cargados:
        st.warning(
            "No hay documentos con preguntas cargadas todavía. Genere algunas preguntas primero."
        )
        st.stop()
...
init_session_state_key("current_quiz_attempt_id", None)
init_session_state_key("questions_presented_in_batch", 0)

try:
    todas_las_preguntas_doc = obtener_preguntas_por_documento(
        conn, documento_id_seleccionado
    )
...
progreso_actual_mostrado = (
    st.session_state[state_prefix + "total_answered_correctly"] / total_preguntas_en_doc
    if total_preguntas_en_doc > 0
    else 0
)
st.progress(
    progreso_actual_mostrado,
    text=f"Progreso: {st.session_state[state_prefix + 'total_answered_correctly']}/{total_preguntas_en_doc} correctas",
)
...
with col_btn1:
    if st.button(
        "Cargar Preguntas",
        key="load_questions_btn",
        help=f"Cargar hasta {NUM_QUESTIONS_PER_BATCH} preguntas no respondidas",
    ):
        try:
            ...
            current_progress_percentage = (
                st.session_state[state_prefix + "total_answered_correctly"]
                / total_preguntas_en_doc
                if total_preguntas_en_doc > 0
                else 0.0
            )
            ...
            st.session_state[state_prefix + "quiz_finished"] = False
            if (
                not preguntas_nuevas
                and st.session_state[state_prefix + "total_answered_correctly"]
                >= total_preguntas_en_doc
            ):
                st.success(
                    "¡Felicidades! Has respondido correctamente a todas las preguntas de este documento."
                )
...
if preguntas_a_mostrar:
    ...
    current_quiz_attempt_id = st.session_state.get(
        state_prefix + "current_quiz_attempt_id"
    )
    if current_quiz_attempt_id is None:
        st.warning(
            "No se ha iniciado un intento de cuestionario. Por favor, carga preguntas primero."
        )
    ...
    with st.form(key="quiz_form"):
        ...
        for i, pregunta in enumerate(preguntas_a_mostrar):
            ...
            try:
                ...
                opciones_mostradas = [
                    f"{letra}) {opcion}"
                    for letra, opcion in opciones_enumeradas.items()
                ]
                ...
                st.error(
                    f"Error al procesar opciones para pregunta {i + 1}. Opciones recibidas: {options_json}"
                )
            except Exception as e:
        ...
        if submitted:
            ...
            if (
                st.session_state[state_prefix + "respuestas_usuario"]
                and current_quiz_attempt_id is not None
            ):
                respuestas_seleccionadas_list = [
                    st.session_state[state_prefix + "respuestas_usuario"].get(i)
                    for i in range(len(preguntas_a_mostrar))
                ]
                ...
                for i, pregunta_actual in enumerate(preguntas_a_mostrar):
                    start_time = st.session_state[state_prefix + "start_times"].get(
                        i, datetime.now()
                    )
                    ...
                    es_correcta = (
                        seleccionada is not None and seleccionada == correcta_letra
                    )
                    ...
                    try:
                        registrar_respuesta_estadistica(
                            ...
                            respuesta_seleccionada=seleccionada if seleccionada else "",
                        )
                    except Exception as e:
                        st.error(
                            f"Error guardando respuesta para pregunta ID {pregunta_actual['id']}: {e}"
                        )
                ...
                try:
                    questions_presented_in_batch = st.session_state[
                        state_prefix + "questions_presented_in_batch"
                    ]
                    correct_in_batch = sum(resultados_bool)

                    incorrect_in_batch = len(resultados_bool) - correct_in_batch
                    ...
                    actualizar_quiz_attempt_final(
                        ...
                        questions_presented=questions_presented_in_batch,
                        questions_answered=num_answered_this_submit,
                ...
                st.session_state[state_prefix + "preguntas_actuales"] = (
                    preguntas_incorrectas_actuales
                )
                st.session_state[state_prefix + "respuestas_usuario"] = {}
                st.session_state[state_prefix + "start_times"] = {}
                ...
                st.error(
                    "Error crítico: No se encontró un ID de intento de cuestionario activo. Por favor, recarga las preguntas."
                )

    resultados_guardados = st.session_state.get(
        state_prefix + "resultados_verificacion", []
    )
    if resultados_guardados:
        ...
        if num_correctas_verificadas > 0:
            st.success(
                f"Respondiste correctamente a {num_correctas_verificadas} de {num_total_verificadas} preguntas verificadas."
            )
        if num_incorrectas_verificadas > 0:
            st.warning(
                f"Tienes {num_incorrectas_verificadas} preguntas incorrectas pendientes en este lote."
            )

        if num_total_verificadas > 0 and num_incorrectas_verificadas == 0:
            st.balloons()
            st.success("¡Todas las preguntas de este lote fueron correctas!")
            if not st.session_state[state_prefix + "preguntas_actuales"]:
                st.info(
                    "Presiona 'Cargar Preguntas' para el siguiente lote o si has terminado."
                )
