    ...
    return AuthConfig.from_mapping(
        {
            ...
            "SARA_AUTH_CALLBACK_URL": "https://examgen.example/auth/callback",
...
def test_callback_defaults_to_the_exact_registered_local_url(tmp_path) -> None:
    ...
    assert config.callback_url == DEFAULT_CALLBACK_URL == "http://localhost:8501/"
...
def test_identity_http_contract_uses_exact_routes_form_and_no_redirects(tmp_path) -> None:
    ...
    assert http.calls[1][2]["headers"]["X-Resource-Audience"] == "examgen"
...
def test_authorization_url_keeps_verifier_only_encrypted_server_side(tmp_path) -> None:
    ...
    assert query["client_id"] == ["examgen-web"]
    assert query["redirect_uri"] == ["https://examgen.example/auth/callback"]
    ...
    assert row[2:4] == ("examgen-web", config.callback_url)
...
def test_identity_uses_uuid_unless_explicit_manifest_maps_legacy_identifier(tmp_path) -> None:
    path = tmp_path / "legacy-identities.json"
    path.write_text(
        json.dumps(
            {
                "version": 1,
                "mappings": {SUBJECT: "legacy.user@example.invalid"},
            }
        ),
        encoding="utf-8",
    )
    manifest = load_identity_manifest(str(path))

    assert data_identity_for_profile(_profile(), manifest) == "legacy.user@example.invalid"
    assert data_identity_for_profile(_profile(OTHER_SUBJECT), manifest) == OTHER_SUBJECT
    assert data_identity_for_profile(
        {"id": OTHER_SUBJECT, "email": "legacy-identifier", "name": "legacy-identifier"},
        manifest,
    ) == OTHER_SUBJECT


def test_admin_policy_uses_only_central_capability_or_explicit_uuid() -> None:
    assert profile_is_admin({**_profile(), "permissions": ["examgen.admin"]})
    assert profile_is_admin({**_profile(), "permissions": ["platform.manage"]})
    assert profile_is_admin(_profile(), parse_admin_subjects([SUBJECT]))
    assert not profile_is_admin(
        {**_profile(), "role": "admin", "permissions": []},
        parse_admin_subjects([OTHER_SUBJECT]),
    )
    with pytest.raises(AuthConfigurationError):
        parse_admin_subjects(["admin@example.invalid"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"not-a-uuid": "legacy-identifier"},
    ],
)
def test_identity_manifest_rejects_unsafe_or_ambiguous_mappings(tmp_path, mappings) -> None:
    path = tmp_path / "legacy.json"
    path.write_text(json.dumps({"version": 1, "mappings": mappings}), encoding="utf-8")

    with pytest.raises(IdentityManifestError):
        load_identity_manifest(str(path))
